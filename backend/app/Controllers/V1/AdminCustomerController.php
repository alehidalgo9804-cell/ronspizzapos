...
final class AdminCustomerController extends Controller
{
    ...
    public function updateAddress(Request $request): void
    {
        ...
        $this->db->prepare(
            'UPDATE direcciones_cliente
             SET alias = :alias, calle = :calle, numero_exterior = :num_ext,
                 numero_interior = :num_int, colonia = :colonia, ciudad = :ciudad,
                 estado = :estado, codigo_postal = :cp, referencia = :referencia,
                 instrucciones_entrega = :instrucciones, activa = :activa, updated_at = NOW()
             WHERE id = :id'
        )->execute([
            'id' => $dirId,
            ...
            'activa' => isset($dir['activa']) ? (int) $dir['activa'] : 1,
        ]);
        $this->ok([], 'Direccion actualizada');
    }
    ...
    {
        $stmt = $this->db->prepare(
            'INSERT INTO direcciones_cliente (cliente_id, alias, calle, numero_exterior, numero_interior, colonia, ciudad, estado, codigo_postal, referencia, instrucciones_entrega, activa, created_at, updated_at)
             VALUES (:cliente_id, :alias, :calle, :num_ext, :num_int, :colonia, :ciudad, :estado, :cp, :referencia, :instrucciones, 1, NOW(), NOW())'
        );
        $stmt->execute([
