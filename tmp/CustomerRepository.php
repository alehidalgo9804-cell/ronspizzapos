    ...
    public function findByPhone(string $phone): ?array
    {
        ...
        $stmt = $this->db->prepare(
            'SELECT *
             FROM clientes
             WHERE (
               REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(telefono, ""), "+", ""), " ", ""), "-", ""), "(", ""), ")", "") = :normalized1
               OR REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(telefono_alterno, ""), "+", ""), " ", ""), "-", ""), "(", ""), ")", "") = :normalized2
             )
             LIMIT 1'
        );
        $stmt->execute(['normalized1' => $normalizedInput, 'normalized2' => $normalizedInput]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
