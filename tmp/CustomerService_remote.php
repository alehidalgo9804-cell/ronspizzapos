...
final class CustomerService extends BaseCrudService
{
    ...
    public function upsertFromPos(array $payload): array
    {
        ...
        $costoEnvioRaw = $payload['costo_envio'] ?? $payload['direccion_costo_envio'] ?? null;
        $costoEnvio = is_numeric($costoEnvioRaw) ? (float) $costoEnvioRaw : null;
        ...
        if ($customerId > 0 && $addressId > 0) {
            ...
            if ($costoEnvio !== null) {
                $addressUpdatePayload['costo_envio'] = $costoEnvio;
            }
